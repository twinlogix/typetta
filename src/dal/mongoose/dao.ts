...
import { DAOAssociationReference, ReadOptions, AbstractDAO, DAOParams, FindParams, FindOneParams, FilterParams, InsertParams, UpdateParams, ReplaceParams, DeleteParams } from '../dao'
...
export abstract class AbstractMongooseDAO<ModelType, IDKey extends keyof Omit<ModelType, ExcludedFields>, IDAutogenerated extends boolean, FilterType, SortType, UpdateType, ExcludedFields extends keyof ModelType> extends AbstractDAO<ModelType, IDKey, IDAutogenerated, FilterType, SortType, UpdateType, ExcludedFields> {
  public DBModel: Model<Document>

  protected constructor({ dbModel, ...params }: { idField: IDKey; daoContext: AbstractDAOContext; dbModel: Model<Document> } & DAOParams<ModelType, IDKey, IDAutogenerated, FilterType, UpdateType, ExcludedFields, SortType>) {
    super(params)
  ...
  protected projectionsToDB(projections?: Projection<ModelType>): any {
    ...
    if (dbProjections && typeof dbProjections === 'object') {
      this.associations.filter((association) => association.reference === DAOAssociationReference.INNER).forEach((association) => addAssociationRefToProjection(association.field, association.refFrom, dbProjections))
  ...
  protected async _find(params: FindParams<FilterType, Projection<ModelType>, SortType>): Promise<PartialDeep<ModelType>[]> {
    if (params.limit === 0) {
      return []
    } else {
      const dbOptions = { skip: params.start, limit: params.limit, sort: this.sortsToDB(params.sorts), ...(params.options || {})._ }
  ...
  protected async _findOne(params: FindOneParams<FilterType, Projection<ModelType>>): Promise<PartialDeep<ModelType> | null> {
    const query = this.DBModel.findOne(this.filterToDB(params.filter), mongooseProjection(this.projectionsToDB(params.projection), this.DBModel.schema), (params.options || {})._)
    return (await this.dbToModel(await query.exec(), this.projectionsToDB(params.projection))) as PartialDeep<ModelType> | null
  }

  protected async _findPage(params: FindParams<FilterType, Projection<ModelType>, SortType>): Promise<{ totalCount: number; records: PartialDeep<ModelType>[] }> {
  ...
  protected async findByQuery<P extends AnyProjection<ModelType>>(query: (dbRef: Model<Document>, dbProjections: any, dbSorts?: any, start?: number, limit?: number, options?: any) => Promise<Document[]>, projection?: P, sorts?: SortType, start?: number, limit?: number, options?: ReadOptions): Promise<ModelProjection<ModelType, P>[]> {
    ...
    if (params.limit === 0) {
      ...
      const dbOptions = { skip: params.start, limit: params.limit, sort: this.sortsToDB(params.sorts), ...(params.options || {})._ }
      const dbRecords = await query(this.DBModel, dbProjections, dbSorts, params.start, params.limit, (params.options || {})._)
  ...
  protected async _exists(params: FilterParams<FilterType>): Promise<boolean> {
    return this.DBModel.exists(this.filterToDB(params.filter))
  }

  protected async _count(params: FilterParams<FilterType>): Promise<number> {
    return this.DBModel.count(this.filterToDB(params.filter)).exec()
  }

  protected async _insertOne(params: InsertParams<ModelType, IDKey, ExcludedFields, IDAutogenerated>): Promise<Omit<ModelType, ExcludedFields>> {
    const dbObjects = await this.DBModel.create([this.modelToDb(params.record)], (params.options || {})._)
    params.record[this.idField] = (dbObjects[0].toObject() as any)[this.idField]
    return params.record as Omit<ModelType, ExcludedFields>
  }

  protected async _updateOne(params: UpdateParams<FilterType, UpdateType>): Promise<void> {
    if (params.options && params.options._) {
      await this.DBModel.updateOne(this.filterToDB(params.filter), this.updatesToDB(params.changes), params.options._).exec()
    } else {
      const res = await this.DBModel.updateOne(this.filterToDB(params.filter), this.updatesToDB(params.changes)).exec()
    }
  }

  protected async _updateMany(params: UpdateParams<FilterType, UpdateType>): Promise<void> {
    if (params.options && params.options._) {
      await this.DBModel.updateMany(this.filterToDB(params.filter), this.updatesToDB(params.changes), params.options._).exec()
    } else {
      await this.DBModel.updateMany(this.filterToDB(params.filter), this.updatesToDB(params.changes)).exec()
    }
  }

  protected async _replaceOne(params: ReplaceParams<FilterType, ModelType, ExcludedFields>): Promise<void> {
    await this.DBModel.replaceOne(params.filter, this.modelToDb(params.replace))
  }

  protected async _deleteOne(params: DeleteParams<FilterType>): Promise<void> {
    if (params.options && params.options._) {
      await this.DBModel.deleteOne(this.filterToDB(params.filter), params.options._).exec()
    } else {
      await this.DBModel.deleteOne(this.filterToDB(params.filter)).exec()
    }
  }

  protected async _deleteMany(params: DeleteParams<FilterType>): Promise<void> {
    if (params.options && params.options._) {
      await this.DBModel.deleteMany(this.filterToDB(params.filter), params.options._).exec()
    } else {
