...
import {
    ...
    ReadOptions, WriteOptions, SuperClassDAO, AbstractDAO, DAOParams,
} from "./dao";
...
export abstract class AbstractMongooseSuperClassDAO
    <
    ModelType,
    IDKey extends keyof Omit<ModelType, ExcludedFields>,
    IDAutogenerated extends boolean,
    ...
    protected subclasses: { typename: string, dbname: string, fields: string[] }[];

    protected constructor({ subclasses, ...params }: { idField: IDKey, daoContext: AbstractDAOContext<SecurityContext>, dbModel: Model<Document>, subclasses: { typename: string, dbname: string, fields: string[] }[] } & DAOParams<ModelType, IDKey, IDAutogenerated, FilterType, UpdateType, ExcludedFields, SecurityContext>) {
        super(params);
        this.subclasses = subclasses;
    }

    protected projectionsToDB(projections?: any): any {
        const dbProjections = super.projectionsToDB(projections);
        if (dbProjections && typeof dbProjections === 'object') {
            dbProjections.__typename = true;
        }
        return dbProjections;
    }

    protected modelToDb(
        model: ModelType | PartialDeep<ModelType> | ConditionalPartialBy<ModelType, IDKey, IDAutogenerated>
    ): any {

        const dbObject = super.modelToDb(model);

        if (dbObject.__typename) {
            const subclass = this.subclasses.find(s => s.typename === dbObject.__typename);
            if (subclass) {
                dbObject[subclass.dbname] = {};
                subclass.fields.forEach(field => {
                    dbObject[subclass.dbname][field] = dbObject[field];
    ...
    protected async dbToModels(dbRecords: (Document | null)[], projections?: Projection<ModelType>): Promise<ModelType[]> {
        const dbObjects = await super.dbToModels(dbRecords, projections);
        return dbObjects.map((dbObject: any) => {
            if (dbObject && dbObject.__typename) {
                const subclass = this.subclasses.find(sub => sub.typename === dbObject.__typename);
                if (subclass) {
                    dbObject = { ...dbObject, ...dbObject[subclass.dbname] };
                    delete dbObject[subclass.dbname];
                }
            }
            return dbObject;
        });
...
// tslint:disable-next-line: max-classes-per-file
export abstract class AbstractMongooseSubClassDAO
    <
    ModelType,
    IDKey extends keyof Omit<ModelType, ExcludedFields>,
    IDAutogenerated extends boolean,
    ...
    protected subclassTypename: string;
    protected subclassDBName: string;
    protected subclassFields: string[];

    protected constructor({ subclassTypename, subclassDBName, subclassFields, ...params }: { idField: IDKey, daoContext: AbstractDAOContext<SecurityContext>, dbModel: Model<Document>, subclassTypename: string, subclassDBName: string, subclassFields: string[], } & DAOParams<ModelType, IDKey, IDAutogenerated, FilterType, UpdateType, ExcludedFields, SecurityContext>) {
        super(params);
        this.subclassTypename = subclassTypename;
        this.subclassDBName = subclassDBName;
    ...
    protected modelToDb(
        ...
        dbObject.__typename = this.subclassTypename;
        dbObject[this.subclassDBName] = {};
        this.subclassFields.forEach(field => {
            dbObject[this.subclassDBName][field] = dbObject[field];
            delete dbObject[field];
        });
        return dbObject;
    }
    ...
    protected filterToDB(filter?: FilterType): any {
        let dbFilter = deepCopy(super.filterToDB(filter));
        if (!dbFilter) {
            dbFilter = {};
        }
        dbFilter.__typename = this.subclassTypename;
        this.transformSubclassFilter(dbFilter);
        return dbFilter;
    }

    protected transformSubclassFilter(filter: any) {
        for (const filterKey in filter) {
            if (filter.hasOwnProperty(filterKey)) {
                for (const subclassField of this.subclassFields) {
                    if (filterKey === subclassField || filterKey.startsWith(subclassField + ".")) {
                        filter[this.subclassDBName + "." + filterKey] = filter[filterKey];
        ...
        for (const filterKey in filter) {
            if (typeof filter[filterKey] === 'object') {
                this.transformSubclassFilter(filter[filterKey]);
            }
        }
    }

    protected projectionsToDB(projections?: any): any {
        const dbProjections = deepCopy(projections);
        if (dbProjections && typeof dbProjections === 'object') {
            dbProjections.__typename = true;
            this.subclassFields.forEach(field => {
                if (dbProjections[field]) {
                    if (!dbProjections[this.subclassDBName]) dbProjections[this.subclassDBName] = {};
                    dbProjections[this.subclassDBName][field] = dbProjections[field];
                    delete dbProjections[field];
                }
            });
        }
        return super.projectionsToDB(dbProjections);
    }

    protected sortsToDB(sorts?: SortType): any {
        const dbSorts = super.sortsToDB(sorts);
        if (dbSorts) {
            for (const sortKey in dbSorts) {
                if (dbSorts.hasOwnProperty(sortKey)) {
                    for (const subclassField of this.subclassFields) {
                        if (sortKey.startsWith(subclassField)) {
                            dbSorts[this.subclassDBName + "." + sortKey] = dbSorts[sortKey];
                            delete dbSorts[sortKey];
                        }
                    }
                }
            }
    ...
    protected updatesToDB(updates: UpdateType): any {
        const dbUpdates = super.updatesToDB(updates);
        const res = { ...dbUpdates };
        for (const updateKey in dbUpdates) {
            if (dbUpdates.hasOwnProperty(updateKey)) {
                for (const subclassField of this.subclassFields) {
                    if (updateKey.startsWith(subclassField)) {
                        res[this.subclassDBName + "." + updateKey] = dbUpdates[updateKey];
                        delete res[updateKey];
    ...
    protected async dbToModels(dbRecords: (Document | null)[], projections?: Projection<ModelType>): Promise<ModelType[]> {
        ...
        return dbObjects.map((dbObject: any) => {
            if (dbObject && dbObject.__typename) {
                const newDBObject = { ...dbObject, ...dbObject[this.subclassDBName] };
                delete newDBObject[this.subclassDBName];
                return newDBObject;
            }
            return dbObject;
        });
