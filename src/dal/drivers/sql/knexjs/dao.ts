...
> extends AbstractDAO<ModelType, IDKey, IDAutogenerated, FilterType, SortType, UpdateType, ExcludedFields, OptionsType> {
  ...
  protected constructor({
    collectionName,
    ...params
  }: { idField: IDKey; daoContext: AbstractDAOContext; collectionName: string } & DAOParams<ModelType, IDKey, IDAutogenerated, FilterType, UpdateType, ExcludedFields, SortType, OptionsType>) {
    super(params)
  ...
  protected _find(params: FindParams<FilterType, Projection<ModelType>, SortType, OptionsType>): Promise<PartialDeep<ModelType>[]> {
    Object.entries(params.filter || {}).forEach(([s, v]) => {
      
    })
    this.knexInstance(this.collectionName).select('*').where('')
