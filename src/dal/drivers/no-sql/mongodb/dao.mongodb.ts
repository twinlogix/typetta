...
export abstract class AbstractMongoDAO<
  ModelType,
  ...
  protected constructor(params: { collection: Collection, idField: IDKey; daoContext: AbstractDAOContext; } & DAOParams<ModelType, IDKey, IDAutogenerated, FilterType, UpdateType, ExcludedFields, SortType, OptionsType>) {
    super(params)
  ...
  protected async _find(params: FindParams<FilterType, Projection<ModelType>, SortType, OptionsType>): Promise<PartialDeep<ModelType>[]> {
    if (params.limit === 0) {
      ...
      // const query = this.collection.find(this.filterToDB(params.filter), mongooseProjection(this.projectionsToDB(params.projection), this.collection.schema), dbOptions)
      // return (await this.dbToModels(await query, this.projectionsToDB(params.projection))) as PartialDeep<ModelType>[]
      // TODO
      return [];
    }
  }
  ...
  protected async _findOne(params: FindOneParams<FilterType, Projection<ModelType>, OptionsType>): Promise<PartialDeep<ModelType> | null> {
    // const query = this.collection.findOne(this.filterToDB(params.filter), mongooseProjection(this.projectionsToDB(params.projection), this.collection.schema), (params.options || {}).mongoose)
    // return (await this.dbToModel(await query, this.projectionsToDB(params.projection))) as PartialDeep<ModelType> | null
    // TODO
    return null;
  }
  ...
  public async findByQuery<P extends AnyProjection<ModelType>>(
    query: (dbRef: Collection, dbProjections: any, dbSorts?: any, start?: number, limit?: number, options?: any) => Promise<Document[]>,
    projection?: P,
    sorts?: SortType,
    start?: number,
    limit?: number,
    ...
    if (params.limit === 0) {
      ...
      // const dbOptions = { skip: params.start, limit: params.limit, sort: this.sortsToDB(params.sorts), ...(params.options || {}).mongoose }
      const dbRecords = await query(this.collection, dbProjections, dbSorts, params.start, params.limit, (params.options || {}).mongoose)
  ...
  protected async _exists(params: FilterParams<FilterType, OptionsType>): Promise<boolean> {
    // TODO
    // return this.collection.exists(this.filterToDB(params.filter))
    return false;
  }

  protected async _count(params: FilterParams<FilterType, OptionsType>): Promise<number> {
    // TODO
    // return this.collection.count(this.filterToDB(params.filter))
    return 0;
  }
  ...
  protected async _insertOne(params: InsertParams<ModelType, IDKey, ExcludedFields, IDAutogenerated, OptionsType>): Promise<Omit<ModelType, ExcludedFields>> {
    const dbObjects = await this.collection.insertOne(this.modelToDb(params.record), (params.options || {}).mongoose)
  ...
  protected async _updateOne(params: UpdateParams<FilterType, UpdateType, OptionsType>): Promise<void> {
    if (params.options && params.options.mongoose) {
      await this.collection.updateOne(this.filterToDB(params.filter), this.updatesToDB(params.changes), params.options.mongoose)
    } else {
      const res = await this.collection.updateOne(this.filterToDB(params.filter), this.updatesToDB(params.changes))
    }
  }

  protected async _updateMany(params: UpdateParams<FilterType, UpdateType, OptionsType>): Promise<void> {
    if (params.options && params.options.mongoose) {
      await this.collection.updateMany(this.filterToDB(params.filter), this.updatesToDB(params.changes), params.options.mongoose)
    } else {
      await this.collection.updateMany(this.filterToDB(params.filter), this.updatesToDB(params.changes))
    }
  }

  protected async _replaceOne(params: ReplaceParams<FilterType, ModelType, ExcludedFields, OptionsType>): Promise<void> {
    await this.collection.replaceOne(params.filter, this.modelToDb(params.replace))
  }

  protected async _deleteOne(params: DeleteParams<FilterType, OptionsType>): Promise<void> {
    if (params.options && params.options.mongoose) {
      await this.collection.deleteOne(this.filterToDB(params.filter), params.options.mongoose)
    } else {
      await this.collection.deleteOne(this.filterToDB(params.filter))
    }
  }

  protected async _deleteMany(params: DeleteParams<FilterType, OptionsType>): Promise<void> {
    if (params.options && params.options.mongoose) {
      await this.collection.deleteMany(this.filterToDB(params.filter), params.options.mongoose)
    } else {
      await this.collection.deleteMany(this.filterToDB(params.filter))
    }
  }
}
