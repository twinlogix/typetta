...
export abstract class AbstractDAO<T extends DAOGenerics> implements DAO<T> {
  ...
  protected constructor({
    ...
    if (this.idGeneration === 'generator' && !this.idGenerator && !this.idScalarGenerator) {
      throw new Error(`ID generator for scalar ${idScalar.toString()} is missing. Define one in EntityManager or in DAOParams.`)
    }
    Object.entries(schema)
      ...
      .forEach(([key, scalar]) => {
        if (!entityManager.adapters[this._driver()][scalar].generate) {
          throw new Error(`Generator for scalar ${scalar.toString()} is needed for generate default fields ${key}. Define one in EntityManager or in DAOParams.`)
        }
      })
  ...
  async findAll<P extends AnyProjection<T['projection']> | GraphQLResolveInfo>(params: FindParams<T, P> = {}): Promise<Project<T['entity'], T['ast'], T['scalars'], P, T['types']>[]> {
    return this.logOperation('findAll', params, async () => {
      ...
      const resolvedRecords = await this.internalResolveRelations(
        records,
  ...
  public async resolveRelations<I extends T['plainModel'] | T['plainModel'][], P extends AnyProjection<T['projection']> | GraphQLResolveInfo>(
    params: {
      input: I
      projection: P
    ...
    const res = await this.internalResolveRelations(
      (Array.isArray(params.input) ? params.input : [params.input]) as PartialDeep<T['model']>[],
      newParams.projection,
      params.relations,
      operationId,
      undefined,
      params.options,
  ...
  private async internalResolveRelations(
    ...
    projections: AnyProjection<T['projection']> | undefined,
    relations: T['relations'] | undefined,
    operationId: string | undefined,
    relationParents: FindParams<T>['relationParents'] | undefined,
    options: T['driverFindOptions'] | undefined,
    metadata: T['operationMetadata'] | undefined,
