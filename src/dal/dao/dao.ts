  ...
  > implements DAO<ModelType, IDKey, IDAutogenerated, FilterType, ProjectionType, SortType, UpdateType, ExcludedFields, OptionsType>
{
  ...
  protected middlewares: DAOMiddleware<ModelType, IDKey, IDAutogenerated, FilterType, ProjectionType, UpdateType, ExcludedFields, SortType, OptionsType & DriverOptionType, ScalarsType>[]
  ...
  protected constructor({
    ...
    this.middlewares = [
      {
        beforeFind: async (params) => this.addOptions(params, this.driverOptions, this.options),
        beforeInsert: async (params) => this.addOptions(params, this.driverOptions, this.options),
        beforeUpdate: async (params) => this.addOptions(params, this.driverOptions, this.options),
        beforeReplace: async (params) => this.addOptions(params, this.driverOptions, this.options),
        beforeDelete: async (params) => this.addOptions(params, this.driverOptions, this.options),
      },
      {
    ...
    this.schema = schema
    this.apiV1 = new DAOWrapperAPIv1<ModelType, IDKey, IDType, IDAutogenerated, FilterType, ProjectionType, SortType, UpdateType, ExcludedFields, OptionsType, DriverOptionType, ScalarsType>(this, idField)
  }

  protected async beforeFind(params: FindParams<FilterType, ProjectionType, SortType, OptionsType & DriverOptionType>): Promise<FindParams<FilterType, ProjectionType, SortType, OptionsType & DriverOptionType>> {
  ...
  protected async afterFind(params: FindParams<FilterType, ProjectionType, SortType, OptionsType & DriverOptionType>, result: PartialDeep<ModelType>): Promise<PartialDeep<ModelType>> {
    const contextOptions = this.createContextOptions()
  ...
  private async elabRecords<P extends AnyProjection<ModelType, ProjectionType>>(
    params: FindParams<FilterType, P, SortType, OptionsType & DriverOptionType>,
    records: PartialDeep<ModelType>[],
  ): Promise<ModelProjection<ModelType, P>[]> {
    const results = []
    for (const record of records) {
      results.push(await this.afterFind(params as FindParams<FilterType, ProjectionType, SortType, OptionsType & DriverOptionType>, record))
    }
    return results as ModelProjection<ModelType, P>[]
  }

  async findAll<P extends AnyProjection<ModelType, ProjectionType> = true>(params: FindParams<FilterType, P, SortType, OptionsType & DriverOptionType> = {}): Promise<ModelProjection<ModelType, P>[]> {
    ...
    return this.elabRecords(newParams as FindParams<FilterType, P, SortType, OptionsType & DriverOptionType>, resolvedRecors)
  }

  async findOne<P extends AnyProjection<ModelType, ProjectionType> = true>(params: FindOneParams<FilterType, P, OptionsType & DriverOptionType> = {}): Promise<ModelProjection<ModelType, P> | null> {
    const newParams = await this.beforeFind({ ...(params as FindParams<FilterType, ProjectionType, SortType, OptionsType & DriverOptionType>) })
    ...
    if (record) {
      ...
      return (await this.elabRecords({ ...(newParams as FindParams<FilterType, P, SortType, OptionsType & DriverOptionType>) }, resolvedRecors))[0]
    }
    return null
  }
  ...
  async exists(params: FilterParams<FilterType, OptionsType & DriverOptionType>): Promise<boolean> {
    const newParams = await this.beforeFind({ ...params })
    return this._exists(newParams)
  }

  async count(params: FilterParams<FilterType, OptionsType & DriverOptionType> = {}): Promise<number> {
    const newParams = await this.beforeFind({ ...params })
  ...
  private async afterInsert(params: InsertParams<ModelType, IDKey, ExcludedFields, IDAutogenerated, OptionsType & DriverOptionType>, result: Omit<ModelType, ExcludedFields>): Promise<Omit<ModelType, ExcludedFields>> {
    const contextOptions = this.createContextOptions()
    for (const middleware of reversed(this.middlewares)) {
  ...
  async insertOne(params: InsertParams<ModelType, IDKey, ExcludedFields, IDAutogenerated, OptionsType & DriverOptionType>): Promise<Omit<ModelType, ExcludedFields>> {
  ...
  private async beforeUpdate(params: UpdateParams<FilterType, UpdateType, OptionsType & DriverOptionType>): Promise<UpdateParams<FilterType, UpdateType, OptionsType & DriverOptionType>> {
    const contextOptions = this.createContextOptions()
    for (const middleware of this.middlewares) {
  ...
  async updateOne(params: UpdateParams<FilterType, UpdateType, OptionsType & DriverOptionType>): Promise<void> {
    const newParams = await this.beforeUpdate(params)
    await this._updateOne(newParams)
    await this.afterUpdate(newParams)
  }

  async updateAll(params: UpdateParams<FilterType, UpdateType, OptionsType & DriverOptionType>): Promise<void> {
    const newParams = await this.beforeUpdate(params)
    await this._updateMany(newParams)
    await this.afterUpdate(newParams)
  }

  private async beforeReplace(params: ReplaceParams<FilterType, ModelType, ExcludedFields, OptionsType & DriverOptionType>): Promise<ReplaceParams<FilterType, ModelType, ExcludedFields, OptionsType & DriverOptionType>> {
    const contextOptions = this.createContextOptions()
    for (const middleware of this.middlewares) {
  ...
  async replaceOne(params: ReplaceParams<FilterType, ModelType, ExcludedFields, OptionsType & DriverOptionType>): Promise<void> {
    const newParams = await this.beforeReplace(params)
    await this._replaceOne(newParams)
    await this.afterReplace(newParams)
  }

  private async beforeDelete(params: DeleteParams<FilterType, OptionsType & DriverOptionType>): Promise<DeleteParams<FilterType, OptionsType & DriverOptionType>> {
    const contextOptions = this.createContextOptions()
    for (const middleware of this.middlewares) {
  ...
  async deleteOne(params: DeleteParams<FilterType, OptionsType & DriverOptionType>): Promise<void> {
    const newParams = await this.beforeDelete(params)
    await this._deleteOne(newParams)
    await this.afterDelete(newParams)
  }

  async deleteAll(params: DeleteParams<FilterType, OptionsType & DriverOptionType>): Promise<void> {
    const newParams = await this.beforeDelete(params)
    await this._deleteMany(newParams)
    await this.afterDelete(newParams)
  }

  private addOptions<T extends { options?: OptionsType & DriverOptionType }>(params: T, driverOptions: DriverOptionType, options?: OptionsType): T {
