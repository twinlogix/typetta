...
export function versioningGraphQLMiddleware<T extends { request: Request, response: Response}>(
    { current, lastSupported } : { current: string, lastSupported: string }) {
    ...
    return async ({ context } : { context: T }, next: () => Promise<any> ) : Promise<any> => {
        ...
        if(!valid(requestAcceptVersion)){
            throw new ApolloError(`Accept-Version header ${requestAcceptVersion} is not a valid semantic version string. Please refer to https://semver.org/ specifications and retry with a valid version.`, "MALFORMED_VERSION", details)
        }
        ...
        if(lt(requestAcceptVersion, lastSupported)){
            throw new ApolloError(`Accept-Version header ${requestAcceptVersion} is less than last supported version ${lastSupported}. Please upgrade your client to a recent version.`, "UNSUPPORTED_VERSION", details)
        }
