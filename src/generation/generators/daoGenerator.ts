...
export class TsTypettaDAOGenerator extends TsTypettaAbstractGenerator {
  ...
  public _generateDAOParams(node: TsTypettaGeneratorNode, typesMap: Map<string, TsTypettaGeneratorNode>): string {
    ...
    const daoAllParams = `type ${node.name}DAOAllParams = ${dbDAOParams}<types.${node.name}, '${idField.name}', ${idField.type}, ${idField.isAutogenerated ? 'true' : 'false'}, ${node.name}Filter, ${
      node.name
  ...
  public _generateDAO(node: TsTypettaGeneratorNode, typesMap: Map<string, TsTypettaGeneratorNode>): string {
    ...
    return (
      `export class ${node.name}DAO extends ${daoName}<types.${node.name}, '${idField.name}', ${idField.type}, ${idField.isAutogenerated ? 'true' : 'false'}, ${node.name}Filter, ${
        node.name
