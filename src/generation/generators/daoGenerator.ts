...
export class TsTypettaDAOGenerator extends TsTypettaAbstractGenerator {
  ...
  public _generateDAOParams(node: TsTypettaGeneratorNode, typesMap: Map<string, TsTypettaGeneratorNode>): string {
    ...
    const daoParams = `export type ${node.name}DAOParams = ${dbDAOParams}<types.${node.name}, '${idField.name}', ${idField.isAutogenerated ? 'true' : 'false'}, ${node.name}Filter, ${node.name}Projection, ${node.name}Update, ${node.name}ExcludedFields, ${node.name}Sort, ${this._getOptionsType(node)}, types.Scalars>;`
    return daoParams
  }
