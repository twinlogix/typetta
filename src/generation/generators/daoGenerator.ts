...
export class TsTypettaDAOGenerator extends TsTypettaAbstractGenerator {
    ...
    return node.fields
      ...
      .map((field) => {
        if (typeof field.type === 'string') {
          ...
          return [`'${field.name}': { embedded: {${embeddedFields}}${field.isRequired ? ', required: true' : ''}${field.isList ? ', array: true' : ''}}`]
        }
        return []
      })
      .reduce((a, c) => [...a, ...c], [])
  ...
  public _generateDAOParams(node: TsTypettaGeneratorNode, typesMap: Map<string, TsTypettaGeneratorNode>): string {
    ...
    const daoParams = `export interface ${node.name}DAOParams extends DAOParams<types.${node.name}, '${idField.name}', ${idField.isAutogenerated ? 'true' : 'false'}, ${node.name}Filter, ${node.name}Projection, ${node.name}Update, ${node.name}ExcludedFields, ${node.name}Sort, ${this._getOptionsType(node)}>{}`
    return daoParams
  }
  ...
  private _generateConstructorMethod(node: TsTypettaGeneratorNode, typesMap: Map<string, TsTypettaGeneratorNode>): string {
    ...
    const constructorBody = `super({ ${indentMultiline(`\n...params, idField: '${idField.name}', \n${associations}`)} \n});`
    return `public constructor(params: MongoDBDAOParams){\n` + indentMultiline(constructorBody) + '\n}'
  }
