...
type SelectProjection<
  M,
  P1 extends StaticProjection<M> | Projection<M>,
  P2 extends StaticProjection<M> | Projection<M>
> = P1 extends StaticGenericProjection
  ? P2 extends StaticGenericProjection
    ? MergeGenericProjection<P1, P2>
    : Projection<M>
  : Projection<M>;

export function mergeGenericProjection<
  P1 extends GenericProjection,
  P2 extends GenericProjection
>(p1: P1, p2: P2): MergeGenericProjection<P1, P2> {
...
export function projection<M>(): ProjectionBuilderInterface<M> {
  return new (class implements ProjectionBuilderInterface<M> {
    ...
    merge<
      P1 extends StaticProjection<M> | Projection<M>,
      P2 extends StaticProjection<M> | Projection<M>
    >(p1: P1, p2: P2): SelectProjection<M, P1, P2> {
      //@ts-ignore
      return mergeProjections(p1, p2);
    }
    ...
    fromInfo(
      ...
      return infoToProjection(
        info,
        defaults,
        context ? context : info.fieldNodes[0],
...
>(p1: P1, p2: P2): MergeGenericProjection<P1, P2> {
  ...
  if (p1 === null || p1 === undefined)
    return p2 as MergeGenericProjection<P1, P2>;
  if (p2 === null || p2 === undefined)
    return p1 as MergeGenericProjection<P1, P2>;
              ...
              if (projection[selection.name.value] === true) {
                ...
                if (
                  ...
                  if (field) {
                    ...
                    return {
                      ...projection,
                      [selection.name.value]: infoToProjection(
                        info,
                        projection[selection.name.value],
                        selection,
                        fieldType,
                        schema
                      ),
...
function fragmentToProjections(
  info: any,
  projection: any,
  selection: any,
  fragmentRef: string,
  ...
  if (projection[fragmentKey] === true) {
    ...
    if (fragmentType === type) {
      ...
      const fragmentProjections: any = infoToProjection(
        info,
        projection[fragmentKey],
        selection,
        fragmentType,
...
export function isProjectionIntersected(
  p1: GenericProjection,
  p2: GenericProjection,
  ignoredKeys: string[] = [],
  ...
  for (var key of Array.from(keySet.values())) {
    if (
      ...
      isProjectionIntersected(
        p1[key],
        p2[key],
        dotNotation ? subKeysOf(ignoredKeys, key) : ignoredKeys
      )
    ) {
      return true;
    }
...
export function isProjectionContained(
  container: GenericProjection,
  contained: GenericProjection,
  ignoredKeys: string[] = [],
  dotNotation: boolean = false
  ...
  for (const [key, subContained] of Object.entries(contained)) {
    ...
    if (subContainer === undefined) {
      ...
      const [subResult, subProjection] = isProjectionContained(
        subContainer,
        subContained,
        dotNotation ? subKeysOf(ignoredKeys, key) : ignoredKeys
      );
...
export function isChangesContainedInProjection(
  containerP: GenericProjection,
  changes: object
): boolean {
  if (typeof containerP === "boolean") {
    ...
    for (const [key, value] of Object.entries(changes)) {
      if (
        !containerP[key] ||
        (typeof value === "object" &&
          !isChangesContainedInProjection(containerP[key], value))
      ) {
        return false;
      }
    }
