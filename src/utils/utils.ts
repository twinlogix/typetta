...
export function addAssociationRefToProjection(
    associationFieldPath: string,
    refPath: string,
    projections: any,
    originalProjections: any = projections) {
    ...
    if (associationFieldPathSplitted.length === 1) {
        ...
        if (projections[associationFieldPathSplitted[0]] === true) {
            ...
            addAssociationRefToProjection(
                associationFieldPathSplitted.slice(1).join('.'),
                refPath,
                projections[associationFieldPathSplitted[0]],
                originalProjections);
        }
    }
}
...
export function flattenSorts<SortInput extends { [key: string]: SortDirection | null | undefined }, SortKey extends string>(
    ...
    return sorts?.map(sort => Object.keys(sort)
...
export function hasIdFilter<IDType, Filter extends { id?: IDType | null | ComparisonOperators<IDType> | ElementOperators<IDType> | EvaluationOperators<IDType> }>(conditions: Filter, id: IDType | null): boolean {
    return hasFieldFilter<IDType, 'id', Filter>(conditions, 'id', id);
}

export function hasFieldFilter<FieldType, FieldName extends string, Filter extends { [P in FieldName]?: FieldType | null | ComparisonOperators<FieldType> | ElementOperators<FieldType> | EvaluationOperators<FieldType> }>(conditions: Filter, fieldName: FieldName, id: FieldType | null): boolean {
    return (
        id && conditions[fieldName] && (
            ...
            (
                typeof conditions[fieldName] === 'object' &&
                (conditions[fieldName] as ComparisonOperators<FieldType>).$eq &&
                (conditions[fieldName] as ComparisonOperators<FieldType>).$eq! === id
            )
        )
    ) || false;
}