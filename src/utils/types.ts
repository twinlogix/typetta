 ...
 */
export type StaticProjection<T> = 
  T extends (infer U)[] ? true | StaticProjection<U>
  : T extends object ? { [P in keyof T]?: true | StaticProjection<T[P]> }
  : true
 ...
 */
export type MergeGenericProjection<T1 extends GenericProjection, T2 extends GenericProjection> = 
  T1 extends true ? T1 :
  T2 extends true ? T2 :
  T1 extends false ? T2 :
  T2 extends false ? T1 :
  Expand<{
      [K in (keyof T1) | (keyof T2)]: 
          T1 extends Record<K, any> ? 
              T2 extends Record<K, any> ? 
 ...
 */
export type ModelProjection<M, P extends StaticProjection<M> | Projection<M> | true | undefined> =
  P extends true ? M & { __projection: "all" }
  : P extends undefined ? M & { __projection: "all" }
  : P extends StaticProjection<M> ? Expand<StaticModelProjection<M, P>>
 ...
 */
type StaticModelProjection<M, P extends StaticProjection<M>> = 
  M extends null ? null :
  M extends undefined ? undefined :
  M extends Primitive ? never :
  M extends (infer U)[] ? P extends StaticProjection<U> ? StaticModelProjection<U, P>[] : never :
  M extends object ? Expand<OmitUndefinedAndNeverKeys<{ 
      [K in keyof M]: 
        P extends Record<K, true> ? 
        M[K] : 
          P extends Record<K, StaticProjection<M[K]>> ? 
          StaticModelProjection<M[K], P[K]> 
          : never 
    }>> & { __projection: P } : 
  never;
 ...
 */
type StaticModelProjectionParam<M, P extends StaticProjection<M>> = 
  M extends null ? null :
  M extends undefined ? undefined :
  M extends Primitive ? never :
  M extends (infer U)[] ? P extends StaticProjection<U> ? StaticModelProjectionParam<U, P>[] : never :
  M extends object ? Expand<OmitUndefinedAndNeverKeys<{ 
      [K in keyof M]: 
        P extends Record<K, true> ? 
        M[K] : 
          P extends Record<K, StaticProjection<M[K]>> ? 
          StaticModelProjectionParam<M[K], P[K]> 
          : never 
    }>> & { __projection?: P } : 
  never;

type Truetizer<P> = (P extends true ? true : P extends object ? {
