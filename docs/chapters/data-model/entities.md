...
In Typetta each entity can have one or more fields which are not of scalar type by refer to embedded entities. These referred entities cannot be annotated as `@sql` or `@mongodb`. Here is a simple example:
...
Typetta offers the possibility to define default values for scalar fields of your entities using the the `@default` directive:
...
There are two options you can choose from for defining the default value:
...
Using a [middleware](../client/middlewares.md) you have the possibility to create a default value that depends on `DAOContext` metadata, where you can store, for example, authorization information about the caller.
## Alias
